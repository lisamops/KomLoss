...
var_names <- as.data.frame(c( "Age", "IQ","Instruction duration"))
...
The participants were instructed to train for a total of 18 hours, 1080 minutes. However, the participants ended up with significantly less duration of the instruction. On average the participants trained for `r round(mean((mlm_data %>% filter(time == 1)%>% filter(group != "Control"))$tot_train_time, na.rm = TRUE), digits = 2) ` (*sd* =`r round(sd((mlm_data %>% filter(time == 1)%>% filter(group != "Control"))$tot_train_time, na.rm = TRUE), digits = 2) `) minutes (`r round(mean((mlm_data %>% filter(time == 1)%>% filter(group != "Control"))$tot_train_time, na.rm = TRUE)/60, digits = 2) ` (*sd* =`r round(sd((mlm_data %>% filter(time == 1)%>% filter(group != "Control"))$tot_train_time, na.rm = TRUE)/60, digits = 2) `) hours (*range* =  `r round(range((mlm_data %>% filter(time == 1)%>% filter(group != "Control"))$tot_train_time, na.rm = TRUE), digits = 0) `). There was no difference in instruction duration between the three intervention groups (*H*(`r kruskal_test_TT[["parameter"]]`) = `r kruskal_test_TT[["statistic"]]`, *p* `r p_small(kruskal_test_TT[["p.value"]])` ). The instruction duration for each group can be seen in Figure X.
...
### Hypothesis 1: Training phonic-based reading strategies improves word recognition 
...
### Hypothesis 2: Training comprehension-based reading strategies improves word recognition 
There was no significant interaction between days and intervention for the comprehension-based reading strategy on word recognition (`r apa_print(model_word_3)$full_result$scaledays_center___FALSE_contrast_1vs3`). 
