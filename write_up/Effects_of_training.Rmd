...
library(ggplot2)
library(FSA)
library(lubridate) #to convert age_in_days to years
```
...
## Participants 
Participants had to attend a Swedish special needs school, benefit from AAC, and  be a beginning reader. To be enrolled in the Swedish special needs curriculum, the student needs an ID diagnosis according to the ICD-10. The diagnosis is based on a thorough medical, psychosocial, and psychological assessment. The teachers were instructed to identify students that would benefit from AAC and could not read more than approximately 20 words, which was the operationalization of being a beginning reader. No exclusion criteria were set for additional diagnoses, aetiology of the ID, or age. 
To reach the recruitment goal of at least 120 participants (see pre-registration, Palmqvist et al., 2020), recruitment included two waves, each consisting of two phases. In the first phase of each wave, teachers at special needs schools were contacted by email, asking whether they were interested in participating in the study. When giving a notification of interest, the teacher specified the number of students at their school that fulfilled the inclusion criteria. The teacher also provided information about the age and level of ID of those students, as well as estimated their adaptive and communication skill level. In the second phase of the recruitment process, teachers with an interest in participating in the study were asked to send information about the study to caregivers of potential participants, and for each student that were possible to include after caregivers had signed an informed consent, teachers provided an estimate of the number of words that the student could read. The information provided to the parents and students was written with easy-to-read language and supported with pictures. At the test session, the students preferred communication mode was used (i.e. if the students used AAC that AAC was used also in the test session). The testing was aborted at any sign of discomfort from the student. The study follows the Ethical principles for medical research involving human subjects from the WMA Declaration of Helsinki (World Medical Association, 2013), and approved by the Ethical Review Board, Sweden (2020-06215).  
In the first wave, 62 participants were recruited, and in the second wave, 75 additional participants, with a final sample of 
`r printnum(length(background_raven$id), numerals = T) ` participants 
($n_{girls}$ = `r length((background_raven %>% filter(gender == "Girl"))$id) `, 
$n_{boys}$ = `r length((background_raven %>% filter(gender == "Boy"))$id)`). 
However, one participant was excluded due to testing not being followed as per protocol, and nine because their teacher-estimated level of word reading was higher than the cut-off applied (i.e., 20 words). Further, three participants dropped out of the study after the first testing session. The final sample thus included 
`r printnum(length((mlm_data %>% filter(time == 1))$id), numerals = T) ` participants 
($n_{girls}$ = `r length((mlm_data  %>% filter(time == 1) %>% filter(gender == "Girl"))$id) `, 
$n_{boys}$ = `r length((mlm_data  %>% filter(time == 1) %>% filter(gender == "Boy"))$id)`), 
with a mean age of `r round(mean(time_length(mlm_data$age_in_days, "years")), digits = 1)` 
(SD = `r round(sd(time_length(mlm_data$age_in_days, "years")), digits = 1)` ) 
and mean IQ of 
`r round(mean((mlm_data  %>% filter(time == 1))$IQ, na.rm = T), digits = 0)` 
(SD = `r round(sd((mlm_data  %>% filter(time == 1))$IQ, na.rm = T), digits = 0)` ). 
Data on diagnoses were collected using parental surveys, see Table \@ref(tab:diagnosisTable).


```{r diagnosisTable, dev.args = list(bg = 'white'), include=TRUE, results='asis'}
...
# Allocation of participants to reading instruction condition  
One unsystematic (teaching-as-usual) and three different systematic approaches to reading instructions were applied in the present study: one applying a phonics-based reading strategy (using ALL), one applying a comprehension-based strategy (using Animega-is), and the third applying both strategies, referred to as the combination group (using ALL and Animega-is). The students that received teaching-as-usual comprised a comparison group. Allocation to the comparison group and the phonics-based reading strategy began after the first wave of recruitment, assigning 29 participations to the former, and 31 to the latter condition. An additional three participants from the second wave were allocated to the phonics-based reading strategy. Thirty-five of the remaining participants from the second wave were allocated to the comprehension-based reading instruction, and 31 to the combination group. For practical reasons, allocation was done on a school-basis, , i.e., all participants at one school were in the same group. To make sure that participants assigned to the different groups did not differ on relevant background variables, the information on adaptive and communicative skill, level of ID, and age, provided from the teachers when leaving their notification of interest, was used.
...
# Results  
The control group = `r printnum(length((mlm_data %>% filter(time == 1)%>% filter(group == "Control"))$id), numerals = T) `, the phonic-based group = `r printnum(length((mlm_data %>% filter(time == 1)%>% filter(group == "ALL"))$id), numerals = T) `, the comprehension-based group = `r printnum(length((mlm_data %>% filter(time == 1)%>% filter(group == "Animega-is"))$id), numerals = T) `, and the combination group = `r printnum(length((mlm_data %>% filter(time == 1)%>% filter(group == "Combi"))$id), numerals = T) `, for age, IQ, and instruction duration for each group. The descriptive statistics for all variables on each assessment time can be seen in Table \@ref(tab:descriptivesTable).  

```{r descriptivesTable, dev.args = list(bg = 'white'), include=TRUE, results='asis'}
...
```{r descReadControlTable, dev.args = list(bg = 'white'), include=FALSE, results='asis'}
...
Response_variable <- c("PA", "Word recognition", "Reading comp")
#T1
...
```{r descReadPBTable, dev.args = list(bg = 'white'), include=FALSE, results='asis'}
...
Response_variable <- c("PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(descriptives_all2['PA_Mean',"V1"], descriptives_all2['word_Mean',"V1"], descriptives_all2['DLS_Mean',"V1"])
SDs_t1 <- c(descriptives_all2['PA_SD', "V1"], descriptives_all2['word_SD', "V1"], descriptives_all2['DLS_SD', "V1"])
Range_t1 <- c(descriptives_all2['PA_Range', "V1"], descriptives_all2['word_Range', "V1"], descriptives_all2['DLS_Range', "V1"])
#T2
Means_t2 <- c(descriptives_all2['PA_Mean', "V2"], descriptives_all2['word_Mean', "V2"], descriptives_all2['DLS_Mean', "V2"])
SDs_t2 <- c(descriptives_all2['PA_SD', "V2"], descriptives_all2['word_SD', "V2"], descriptives_all2['DLS_SD', "V2"])
Range_t2 <- c(descriptives_all2['PA_Range', "V2"], descriptives_all2['word_Range', "V2"], descriptives_all2['DLS_Range', "V2"])
#T3
Means_t3 <- c(descriptives_all2['PA_Mean', "V3"], descriptives_all2['word_Mean', "V3"], descriptives_all2['DLS_Mean', "V3"])
SDs_t3 <- c(descriptives_all2['PA_SD', "V3"], descriptives_all2['word_SD', "V3"], descriptives_all2['DLS_SD', "V3"])
Range_t3 <- c(descriptives_all2['PA_Range', "V3"], descriptives_all2['word_Range', "V3"], descriptives_all2['DLS_Range', "V3"])
#T4
Means_t4 <- c(descriptives_all2['PA_Mean', "V4"], descriptives_all2['word_Mean', "V4"], descriptives_all2['DLS_Mean', "V4"])
SDs_t4 <- c(descriptives_all2['PA_SD', "V4"], descriptives_all2['word_SD', "V4"], descriptives_all2['DLS_SD', "V4"])
Range_t4 <- c(descriptives_all2['PA_Range', "V4"], descriptives_all2['word_Range', "V4"], descriptives_all2['DLS_Range', "V4"])
...
```{r descReadAnimegaTable, dev.args = list(bg = 'white'), include=FALSE, results='asis'}
...
Response_variable <- c("PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(descriptives_animega2['PA_Mean',"V1"], descriptives_animega2['word_Mean',"V1"], descriptives_animega2['DLS_Mean',"V1"])
SDs_t1 <- c(descriptives_animega2['PA_SD', "V1"], descriptives_animega2['word_SD', "V1"], descriptives_animega2['DLS_SD', "V1"])
Range_t1 <- c(descriptives_animega2['PA_Range', "V1"], descriptives_animega2['word_Range', "V1"], descriptives_animega2['DLS_Range', "V1"])
#T2
Means_t2 <- c(descriptives_animega2['PA_Mean', "V2"], descriptives_animega2['word_Mean', "V2"], descriptives_animega2['DLS_Mean', "V2"])
SDs_t2 <- c(descriptives_animega2['PA_SD', "V2"], descriptives_animega2['word_SD', "V2"], descriptives_animega2['DLS_SD', "V2"])
Range_t2 <- c(descriptives_animega2['PA_Range', "V2"], descriptives_animega2['word_Range', "V2"], descriptives_animega2['DLS_Range', "V2"])
#T3
Means_t3 <- c(descriptives_animega2['PA_Mean', "V3"], descriptives_animega2['word_Mean', "V3"], descriptives_animega2['DLS_Mean', "V3"])
SDs_t3 <- c(descriptives_animega2['PA_SD', "V3"], descriptives_animega2['word_SD', "V3"], descriptives_animega2['DLS_SD', "V3"])
Range_t3 <- c(descriptives_animega2['PA_Range', "V3"], descriptives_animega2['word_Range', "V3"], descriptives_animega2['DLS_Range', "V3"])
#T4
Means_t4 <- c(descriptives_animega2['PA_Mean', "V4"], descriptives_animega2['word_Mean', "V4"], descriptives_animega2['DLS_Mean', "V4"])
SDs_t4 <- c(descriptives_animega2['PA_SD', "V4"], descriptives_animega2['word_SD', "V4"], descriptives_animega2['DLS_SD', "V4"])
Range_t4 <- c(descriptives_animega2['PA_Range', "V4"], descriptives_animega2['word_Range', "V4"], descriptives_animega2['DLS_Range', "V4"])

descTable3 <- data.frame(Response_variable, Means_t1, SDs_t1, Range_t1, Means_t2, SDs_t2, Range_t2, Means_t3, SDs_t3, Range_t3, Means_t4, SDs_t4, Range_t4)
...
```{r descReadCombiTable, dev.args = list(bg = 'white'), include=FALSE, results='asis'}
...
Response_variable <- c("PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(descriptives_combi2['PA_Mean',"V1"], descriptives_combi2['word_Mean',"V1"], descriptives_combi2['DLS_Mean',"V1"])
SDs_t1 <- c(descriptives_combi2['PA_SD', "V1"], descriptives_combi2['word_SD', "V1"], descriptives_combi2['DLS_SD', "V1"])
Range_t1 <- c(descriptives_combi2['PA_Range', "V1"], descriptives_combi2['word_Range', "V1"], descriptives_combi2['DLS_Range', "V1"])
#T2
Means_t2 <- c(descriptives_combi2['PA_Mean', "V2"], descriptives_combi2['word_Mean', "V2"], descriptives_combi2['DLS_Mean', "V2"])
SDs_t2 <- c(descriptives_combi2['PA_SD', "V2"], descriptives_combi2['word_SD', "V2"], descriptives_combi2['DLS_SD', "V2"])
Range_t2 <- c(descriptives_combi2['PA_Range', "V2"], descriptives_combi2['word_Range', "V2"], descriptives_combi2['DLS_Range', "V2"])
#T3
Means_t3 <- c(descriptives_combi2['PA_Mean', "V3"], descriptives_combi2['word_Mean', "V3"], descriptives_combi2['DLS_Mean', "V3"])
SDs_t3 <- c(descriptives_combi2['PA_SD', "V3"], descriptives_combi2['word_SD', "V3"], descriptives_combi2['DLS_SD', "V3"])
Range_t3 <- c(descriptives_combi2['PA_Range', "V3"], descriptives_combi2['word_Range', "V3"], descriptives_combi2['DLS_Range', "V3"])
#T4
Means_t4 <- c(descriptives_combi2['PA_Mean', "V4"], descriptives_combi2['word_Mean', "V4"], descriptives_combi2['DLS_Mean', "V4"])
SDs_t4 <- c(descriptives_combi2['PA_SD', "V4"], descriptives_combi2['word_SD', "V4"], descriptives_combi2['DLS_SD', "V4"])
Range_t4 <- c(descriptives_combi2['PA_Range', "V4"], descriptives_combi2['word_Range', "V4"], descriptives_combi2['DLS_Range', "V4"])
...
```{r allTables, dev.args = list(bg = 'white'), include=TRUE, results='asis'}
...
Response_variable <- c("Comparison", "PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(" ", descriptives_control2['PA_Mean',"V1"], descriptives_control2['word_Mean',"V1"], descriptives_control2['DLS_Mean',"V1"])
SDs_t1 <- c(" ",descriptives_control2['PA_SD', "V1"], descriptives_control2['word_SD', "V1"], descriptives_control2['DLS_SD', "V1"])
Range_t1 <- c(" ",descriptives_control2['PA_Range', "V1"], descriptives_control2['word_Range', "V1"], descriptives_control2['DLS_Range', "V1"])
...
descTableControl <- data.frame(Response_variable, Means_t1, SDs_t1, Range_t1, Means_t2, SDs_t2, Range_t2, Means_t3, SDs_t3, Range_t3, Means_t4, SDs_t4, Range_t4)
...
Response_variable <- c("Phonic-based" ,"PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(" ",descriptives_all2['PA_Mean',"V1"], descriptives_all2['word_Mean',"V1"], descriptives_all2['DLS_Mean',"V1"])
SDs_t1 <- c(" ",descriptives_all2['PA_SD', "V1"], descriptives_all2['word_SD', "V1"], descriptives_all2['DLS_SD', "V1"])
Range_t1 <- c(" ",descriptives_all2['PA_Range', "V1"], descriptives_all2['word_Range', "V1"], descriptives_all2['DLS_Range', "V1"])
...
descTablePB <- data.frame(Response_variable, Means_t1, SDs_t1, Range_t1, Means_t2, SDs_t2, Range_t2, Means_t3, SDs_t3, Range_t3, Means_t4, SDs_t4, Range_t4)
...
Response_variable <- c("Comprehension-based", "PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(" ",descriptives_animega2['PA_Mean',"V1"], descriptives_animega2['word_Mean',"V1"], descriptives_animega2['DLS_Mean',"V1"])
SDs_t1 <- c(" ",descriptives_animega2['PA_SD', "V1"], descriptives_animega2['word_SD', "V1"], descriptives_animega2['DLS_SD', "V1"])
Range_t1 <- c(" ",descriptives_animega2['PA_Range', "V1"], descriptives_animega2['word_Range', "V1"], descriptives_animega2['DLS_Range', "V1"])
...
descTableCB <- data.frame(Response_variable, Means_t1, SDs_t1, Range_t1, Means_t2, SDs_t2, Range_t2, Means_t3, SDs_t3, Range_t3, Means_t4, SDs_t4, Range_t4)
...
Response_variable <- c("Combination", "PA", "Word recognition", "Reading comp")
#T1
Means_t1 <- c(" ",descriptives_combi2['PA_Mean',"V1"], descriptives_combi2['word_Mean',"V1"], descriptives_combi2['DLS_Mean',"V1"])
SDs_t1 <- c(" ",descriptives_combi2['PA_SD', "V1"], descriptives_combi2['word_SD', "V1"], descriptives_combi2['DLS_SD', "V1"])
Range_t1 <- c(" ",descriptives_combi2['PA_Range', "V1"], descriptives_combi2['word_Range', "V1"], descriptives_combi2['DLS_Range', "V1"])
...
descTableCombi <- data.frame(Response_variable, Means_t1, SDs_t1, Range_t1, Means_t2, SDs_t2, Range_t2, Means_t3, SDs_t3, Range_t3, Means_t4, SDs_t4, Range_t4)
...
apa_table(
  ...
  , caption = "Descriptive statistics for the groups of predictor variables presented by time"
  , note = "this is a note."
  , col_spanners = list("T1" = c(2, 4), "T2" = c(5, 7), "T3" = c(8, 10), "T4" = c(11, 13)) # N.B. ignored in Word
  , col.names = col.names
  , row.names = FALSE
...
The instruction duration for each group can be seen in Figure \@ref(fig:chunk-name)).

```{r  chunk-name, include = TRUE, results='asis', fig.cap = "Duration of instruction in each group"}
...
All groups improved in PA with time, `r apa_print(model_PA_3)$full_result$days_scale`, and the participants in the combination group improved more than the participants in the other groups, `r apa_print(model_PA_3)$full_result$days_scale_contrast_4vs23` . The effect of the phonics-based reading instruction, compared to the comparison group, approached statistical significance, `r apa_print(model_PA_3)$full_result$days_scale_contrast_1vs2`. However, the comprehension-based reading instruction did not lead to better development than the TAU approach in the comparison group, `r apa_print(model_PA_3)$full_result$days_scale_contrast_1vs3`. In sum, we found support for Hypothesis 3, but not for Hypotheses 1-2, see Table \@ref(tab:PATable).   

(ref:est-heading) $\beta$  

```{r PATable, include = TRUE, results='asis'}
tab_model(model_PA_3,
  string.est = "(ref:est-heading)",
  show.re.var= T,            
  show.obs = T,
   ...
   title = "(\\#tab:PATable) Effects of Interventions on Phonological Awareness")
```
...
## Word recognition
For word recognition, participants improved with time, `r apa_print(model_word_3)$full_result$days_scale`. This applied to all four groups, and neither PB (`r apa_print(model_word_3)$full_result$days_scale_contrast_1vs2`), CB (`r apa_print(model_word_3)$full_result$days_scale_contrast_1vs3`), nor the combination of the two instructions (`r apa_print(model_word_3)$full_result$days_scale_contrast_4vs23`) led to a stronger development than what was observed in the comparison group. Thus, for word reading, we did not find support for Hypotheses 1-3, see  Table \@ref(tab:wordTable).


```{r wordTable, include=TRUE, results='asis'}
tab_model(model_word_3,
          string.est = "(ref:est-heading)",
          show.re.var= T,            
          show.obs = T,
                  pred.labels =c("(Intercept)", "Days", "Control vs PB", "Control vs CB", "PB and CB vs Combi", "Days*Control vs PB", "Days*Control vs CB", "Days*PB and CB vs Combi"),
           title = "(\\#tab:wordTable) Effects of Interventions on Word Recognition")
