...
col.names = c(" ", "$M$", "$SD$", "$(Range)$", "$M$", "$SD$", "$(Range)$", "$M$", "$SD$", "$(Range)$", "$M$", "$SD$", "$(Range)$")
...
apa_table(
  ...
  , col_spanners = list("Control" = c(2, 4), "ALL" = c(5, 7), "Animega" = c(8, 10), "Combi" = c(11, 13)) # N.B. ignored in Word
  , col.names = col.names
  , row.names = FALSE
  , align = c('l', 'c')
  , font_size = c("small")
...
# mlm_data <- mlm_data %>% dplyr::mutate(years_old = age_in_days/365)
# 
# desc_data <- mlm_data %>% dplyr::select(-id, -days, - age_in_days, -IQ_scale, -tot_train_time_scale, -contrast_1vs234, -contrast_1vs2, -contrast_1vs3, -contrast_4vs23)
# 
# desc_data$group <- as.numeric(desc_data$group)
# 
# descriptives_control <- desc_data  %>% filter(group == 1) %>% group_by(time) %>%
#    summarize_all(list(Mean=~mean(., na.rm = T),  SD=~sd(., na.rm = T), Range=~paste("(", toString(round(range(., na.rm = T), digits = 0)), ")", sep="")))
#  
# #descriptives[, -1] <- printnum(descriptives[, -1])
...
# 
# Means_t1 <- slice(descriptives_t1, 3:9) 
# SDs_t1 <- slice(descriptives_t1, 11:17) 
# Range_t1 <- slice(descriptives_t1, 19:25) 
# 
# Means_t2 <- slice(descriptives_t2, 3:9) 
# SDs_t2 <- slice(descriptives_t2, 11:17) 
# Range_t2 <- slice(descriptives_t2, 19:25) 
# 
# Means_t3 <- slice(descriptives_t3, 3:9) 
# SDs_t3 <- slice(descriptives_t3, 11:17) 
# Range_t3 <- slice(descriptives_t3, 19:25) 
# 
# Means_t4 <- slice(descriptives_t4, 3:9) 
# SDs_t4 <- slice(descriptives_t4, 11:17) 
# Range_t4 <- slice(descriptives_t4, 19:25) 
#  
# var_names <- as.data.frame(c( "Letter", "Word","Sentence", "IQ", "Training time", "PA", "Age"))
#  
...
There was a significant interaction between time and the intervention on phonemic training on sentence reading (`r apa_print(model_letter_3)$full_result$scaledays_center___FALSE_contrast_1vs2`). 

### Hypothesis 1b: Training comprehension-based reading strategies improves sletter-sound recognition. 
There was no significant interaction between time and intervention for the comprehension-based reading strategy on sentence reading (`r apa_print(model_letter_3)$full_result$scaledays_center___FALSE_contrast_1vs3`). 
